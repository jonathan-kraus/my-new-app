...
export function useLunarTimeline(moonriseStr: string | null, moonsetStr: string | null) {
	const now = useNow();
	...
	const isVisible = normalizedNow >= moonrise && normalizedNow <= adjustedMoonset;
	...
	if (isVisible) {
		...
		progressPercent = Math.min(100, Math.max(0, (elapsed / visibilityMs) * 100));
	}
