...
async function main() {
  ...
  if (!/^#\d+\s*$/.test(currentMessage)) {
    console.log('Log message is not just a counter; skipping. Current message:', JSON.stringify(currentMessage));
    process.exit(0);
  }
  ...
  const newMessage = eventIndex !== undefined && newBody ? `#${eventIndex} ${newBody}` : `#${eventIndex ?? log.id} ${newBody}`;
