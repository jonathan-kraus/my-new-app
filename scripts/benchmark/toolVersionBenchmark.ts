...
async function buildToolEntries(total: number, changeCount: number) {
  ...
  for (let i = 0; i < Math.min(existing.length, total); i++) {
    entries.push({ name: existing[i].name, version: existing[i].version });
  }

  // Add more entries as needed (new names)
  while (entries.length < total) {
    entries.push({ name: `benchmark-tool-${randSuffix()}`, version: `1.0.${Math.floor(Math.random() * 10)}` });
  }
  ...
  for (let i = 0; i < entries.length && changed < changeCount; i++) {
    const e = entries[i];
    // Only change if entry likely exists in DB (heuristic: not our benchmark-tool-...)
...
async function sequentialBenchmark(toolEntries: { name: string; version: string }[], apply: boolean) {
  const start = nowMs();
  let creates = 0;
  let verified = 0;
  let changes = 0;
  ...
  for (const { name, version } of toolEntries) {
    ...
    if (!current) {
      creates++;
      if (apply) {
        await db.toolVersion.create({ data: { name, version, added_at: new Date(), verified_at: new Date() } });
      }
      continue;
    }
    ...
    if (current.version === version) {
      verified++;
      if (apply) {
        await db.toolVersion.update({ where: { name }, data: { verified_at: new Date() } });
      }
      continue;
    }
    ...
    if (apply) {
      await db.toolVersion.upsert({
        where: { name: baseName },
        create: { name: baseName, version: current.version, added_at: current.added_at ?? new Date(), verified_at: new Date() },
        update: { version: current.version, verified_at: new Date() },
      });

      await db.toolVersion.update({ where: { name }, data: { version, added_at: new Date(), verified_at: new Date() } });
    }
  }
...
async function batchedBenchmark(toolEntries: { name: string; version: string }[], apply: boolean) {
  const start = nowMs();
  const names = toolEntries.map((t) => t.name);
  const existing = await db.toolVersion.findMany({ where: { name: { in: names } } });
  const existingMap: Record<string, any> = Object.fromEntries(existing.map((e) => [e.name, e]));
  ...
  for (const { name, version } of toolEntries) {
    ...
    if (!current) {
      toCreate.push({ name, version, added_at: new Date(), verified_at: new Date() });
  ...
  if (apply) {
    await db.$transaction(async (tx) => {
      if (toCreate.length) {
        await tx.toolVersion.createMany({ data: toCreate, skipDuplicates: true });
      }

      if (verifyNames.length) {
        await tx.toolVersion.updateMany({ where: { name: { in: verifyNames } }, data: { verified_at: new Date() } });
      }
      ...
      for (const { name, version, current } of toChange) {
        ...
        await tx.toolVersion.update({ where: { name }, data: { version, added_at: new Date(), verified_at: new Date() } });
      }
    });
  }

  const elapsed = hrElapsed(start);
  return { elapsed, toCreate: toCreate.length, verifyNames: verifyNames.length, toChange: toChange.length };
}

async function main() {
  const argv = parseArgs();
  if (argv.help) {
    console.log("Usage: npx ts-node scripts/benchmark/toolVersionBenchmark.ts --count <N> --changes <M> [--apply]");
    process.exit(0);
  }
  ...
  console.log(`Building ${argv.count} tool entries (with ${argv.changes} simulated changes)...`);
  ...
  console.log(`  creates: ${seqResult.creates}, verified: ${seqResult.verified}, changes: ${seqResult.changes}`);
  ...
  console.log(`  toCreate: ${batResult.toCreate}, verifyNames: ${batResult.verifyNames}, toChange: ${batResult.toChange}`);
