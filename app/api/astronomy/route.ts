...
// ------------------------------------------------------------
function pickCorrectSunrise(snapshot: { sunrise: string; todaySunrise?: string | null }) {
	const now = new Date();

	const sunrise = new Date(snapshot.sunrise);
	const todaySunrise = snapshot.todaySunrise ? new Date(snapshot.todaySunrise) : null;
...
export async function GET(req: NextRequest) {
	try {
		...
		const tomorrowDate = new Date(new Date(localDate).getTime() + 24 * 60 * 60 * 1000)
			.toISOString()
			.slice(0, 10);
...
` as AstronomyRow[];
		...
		if (!todayRow) {
			return NextResponse.json({ error: "No astronomy data for today" }, { status: 404 });
		}
