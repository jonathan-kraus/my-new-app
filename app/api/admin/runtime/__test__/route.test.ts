...
describe("GET /api/admin/runtime", () => {
	...
	it("returns configs mapped to key/value and requests DB ordering", async () => {
		const mocked = [
			{ key: "z.alpha", value: "3", createdAt: new Date(), updatedAt: new Date() },
			{ key: "a.beta", value: "1", createdAt: new Date(), updatedAt: new Date() },
			{ key: "m.gamma", value: "2", createdAt: new Date(), updatedAt: new Date() },
		] as any;
		...
		expect(mockedDb.runtimeConfig.findMany).toHaveBeenCalledWith({ orderBy: { key: "asc" } });
		// And it maps to { key, value } preserving the DB-provided order
		expect(json.configs).toEqual(mocked.map((c: any) => ({ key: c.key, value: c.value })));
	});
});
