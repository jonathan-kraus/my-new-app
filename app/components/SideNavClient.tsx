...
export default function SideNavClient({ nextEventLabel, nextEventTime }: SideNavClientProps) {
	...
	logit(
		"Sidenav",
		{ level: "Info", message: `In SidenavClient using version ${formattedVersion}` },
		...
		<aside className="w-64 h-screen flex flex-col bg-slate-950 text-white shadow-xl">
			...
			<nav className="flex-1 p-4 space-y-2 overflow-y-auto bg-gradient-to-b from-blue-600 via-blue-700 to-transparent">
				...
				<div className="mt-6">
					...
					<NextEventCard nextEvent={nextEventLabel} nextEventTime={nextEventTime} />
				</div>

				<div className="text-xs opacity-60 mt-4">SideNav activations: {activations}</div>

				<div className="text-xs opacity-75 text-yellow-300 mt-1">Version: {formattedVersion}</div>
			</nav>
		</aside>
	);
}
