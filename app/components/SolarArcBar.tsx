...
export function SolarArcBar({
	...
	const position = ((currentTime.getTime() - events.Sunrise.getTime()) / totalSpan) * 100;
	...
	return (
		<div className="relative h-24 w-full rounded-xl bg-gradient-to-r from-blue-900 via-gray-400 to-blue-900 shadow-inner">
			...
			>
				<path d="M0,50 Q50,0 100,50" stroke="white" strokeWidth="0.5" fill="none" />
				<circle
					cx={position}
			...
			{Object.entries(events).map(([label, time]) => {
				const left = ((time.getTime() - events.Sunrise.getTime()) / totalSpan) * 100;
				const labelText = label.replace(/([A-Z])/g, " $1").trim();

				return (
					<div key={label} className="absolute top-0 h-full" style={{ left: `${left}%` }}>
						<div className="w-px h-full bg-white/60" />
						<div className="text-xs text-white absolute top-full mt-1 whitespace-nowrap">
							{labelText}
							<br />
							{formatInTimeZone(time, "America/New_York", "h:mm a")}
						</div>
					</div>
				);
			})}
