...
export default function LiveTimeline({ today, tomorrow }: LiveTimelineProps) {
	...
	const { events, nextEvent, previousEvent, currentEvent, timeUntilNext, timeSincePrevious } =
		...
		<div className="rounded-xl bg-white/5 p-5 border border-white/10 space-y-4">
			...
			{currentEvent && (
				<div className="p-3 rounded-lg bg-blue-500/20 border border-blue-400/30">
					<div className="text-white font-medium">In Progress: {currentEvent.label}</div>
					<div className="text-gray-300 text-sm">
						Started{" "}
						{currentEvent?.at ? formatCountdown(Date.now() - currentEvent.at.getTime()) : "—"} ago
					</div>
				</div>
			)}
			...
			{nextEvent && (
				<div className="p-3 rounded-lg bg-green-500/20 border border-green-400/30">
					<div className="text-white font-medium">Next Event: {nextEvent.label}</div>
					<div className="text-gray-300 text-sm">
				...
				{events.map((e, i) => {
					...
					return (
						...
						>
							...
							<div className="text-gray-400 text-sm">{formatEventTime(e.at)}</div>
						</div>
					);
				})}
			</div>
