...
export default function TimestampDebugPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      console.log("DEBUG TIMESTAMPS RESPONSE:", json);
  ...
  return (
    <div className="p-6 max-w-xl mx-auto space-y-4">
      <h1 className="text-2xl font-bold text-gray-100 mb-4">Timestamp Debug</h1>
      ...
      <TimestampRow
        label="Illumination"
        value={snap.illumination?.toString() ?? ""}
      />
      <TimestampRow label="Phase Name" value={snap.phaseName ?? ""} />
      <TimestampRow
        label="Moon Phase"
        value={snap.moonPhase?.toString() ?? ""}
      />
    </div>
  );
}
