...
export default async function AstronomyPage() {
	...
	if (!location) {
		return <div className="p-8 text-red-300">No default location found in the database.</div>;
	}
	...
	if (!snapshot) {
		return <div className="p-8 text-red-300">No astronomy data found for {location.name}.</div>;
	}
	...
	return (
		<div className="min-h-screen bg-gradient-to-b from-sky-700 to-sky-900 text-white p-8">
			<div className="max-w-5xl mx-auto space-y-8">
				{/* Header */}
				<header>
					<h1 className="text-4xl font-semibold mb-1">Astronomy for {location.name}</h1>
					<p className="text-sky-200">Updated {snapshot.fetchedAt.toLocaleTimeString()}</p>
				</header>
