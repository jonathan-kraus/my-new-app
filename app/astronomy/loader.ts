...
export async function loadAstronomySnapshots() {
	...
	const validated = normalized.map((snap) => {
		...
		const today = validated.find(
			(snap) =>
				snap.date.toLocaleDateString("en-CA", { timeZone: "America/New_York" }) === todayStr,
		);
		const tomorrow = validated.find(
			(snap) =>
				snap.date.toLocaleDateString("en-CA", { timeZone: "America/New_York" }) === tomorrowStr,
		);
    console.log(today, tomorrow)
		if (!parsed.success) {
			console.error("❌ Astronomy snapshot validation failed:", parsed.error.format());
	...
	return {
		...
		tomorrow: tomorrowRaw ? normalizeAstronomySnapshot(tomorrowRaw).tomorrow : null,
	};
}
