...
function KV({ label, value }: { label: string; value: string | number | null | undefined }) {
  return (
    <div className="flex gap-3 text-xs py-[3px]">
      <span className="text-zinc-400 dark:text-zinc-500 min-w-[110px] shrink-0 font-sans">{label}</span>
      <span className={value === null || value === undefined ? 'italic text-zinc-300 dark:text-zinc-600' : 'text-zinc-700 dark:text-zinc-200 break-all'}>
...
export default function LogViewer() {
  ...
  const fetchLogs = useCallback(async (opts?: { reset?: boolean; silent?: boolean }) => {
    ...
    try {
      ...
      data.levels.forEach(r => { lc[r.level] = r.count })
      setLevelCounts(lc)
  ...
  return (
    <div className="flex h-screen bg-zinc-50 dark:bg-zinc-950 font-mono text-sm overflow-hidden">
      ...
      <aside className="w-52 shrink-0 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 flex flex-col py-4 overflow-y-auto">
        <p className="text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 px-4 mb-3">Level</p>

        {[['all', 'All', totalAll], ['error', 'Error', levelCounts.error ?? 0],
          ['warn', 'Warn', levelCounts.warn ?? 0], ['info', 'Info', levelCounts.info ?? 0],
          ['debug', 'Debug', levelCounts.debug ?? 0]].map(([val, label, cnt]) => (
          <button
            key={val}
          ...
          >
            <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${LEVEL_DOT[String(val)] ?? 'bg-zinc-300'}`} />
            {label}
            <span className="ml-auto text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500 px-1.5 py-0.5 rounded-full">{cnt}</span>
          </button>
        ))}

        <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800">
          <p className="text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 px-4 mb-3">Domain</p>
          {[{ domain: 'all', count: totalAll }, ...domains].map(({ domain, count }) => (
            <button
              key={domain}
            ...
            >
              ...
              <span className="ml-auto text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-500 px-1.5 py-0.5 rounded-full">{count}</span>
            </button>
          ))}
        </div>
      </aside>
      ...
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
          ...
          <select
            value={window_}
            onChange={e => { if (!tailing) setWindow(e.target.value) }}
            disabled={tailing}
          ...
          <div className="flex items-center gap-1.5">
            ...
            {!tailing && (
              ...
              >
                {TAIL_INTERVALS.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            )}
            ...
            <button
              ...
              title={tailing ? `Live — next refresh in ${countdown}s` : 'Start live tail'}
            >
              {tailing ? (
                <>
                  {/* Pulsing dot */}
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="sticky top-0 z-10 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
                  <th className="text-left text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 px-3 py-2 w-36">Timestamp</th>
                  <th className="text-left text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 px-3 py-2 w-16">Level</th>
                  <th className="text-left text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 px-3 py-2">Message</th>
                  <th className="text-left text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 px-3 py-2 w-28">Domain</th>
                  <th className="text-left text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 px-3 py-2 w-28">User</th>
                </tr>
              </thead>
              <tbody>
                {logs.map(log => (
                  ...
                  >
                    <td className="px-3 py-1.5 text-[11px] text-zinc-400 whitespace-nowrap">{fmtTs(log.created_at)}</td>
                    <td className="px-3 py-1.5">
                      <span className={`text-[10px] font-sans font-medium px-1.5 py-0.5 rounded ${LEVEL_BADGE[log.level] ?? LEVEL_BADGE.debug}`}>
                        {log.level}
                      </span>
                    </td>
                    <td className="px-3 py-1.5 text-zinc-700 dark:text-zinc-300 max-w-xs truncate">{log.message}</td>
                    <td className="px-3 py-1.5 text-[11px] text-zinc-400 whitespace-nowrap">{log.domain}</td>
                ...
                {!loading && logs.length === 0 && (
                  <tr><td colSpan={5} className="text-center py-16 text-zinc-400 font-sans text-sm">No logs found</td></tr>
                )}
              </tbody>
            </table>
          </div>
            ...
            <aside className="w-96 shrink-0 bg-white dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 flex flex-col overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 shrink-0">
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-sans font-medium px-1.5 py-0.5 rounded ${LEVEL_BADGE[selectedLog.level] ?? LEVEL_BADGE.debug}`}>
                    {selectedLog.level}
                  </span>
                  <span className="text-xs font-sans text-zinc-400">#{selectedLog.id}</span>
                </div>
                <button onClick={() => setSelectedLog(null)} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200">
              ...
              <div className="flex-1 overflow-y-auto">
                <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800">
                  <p className="text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 mb-2">Core</p>
                  ...
                  <KV label="file" value={selectedLog.file ? `${selectedLog.file}${selectedLog.line ? ':' + selectedLog.line : ''}` : null} />
                </div>

                <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800">
                  <p className="text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 mb-2">Session</p>
                ...
                <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800">
                  <p className="text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 mb-2">Payload</p>
                  <JsonBlock value={selectedLog.payload} />
                </div>

                {selectedLog.meta && (
                  <div className="px-4 py-3">
                    <p className="text-[10px] font-sans font-medium tracking-widest uppercase text-zinc-400 mb-2">Meta</p>
        ...
        <div className="flex items-center gap-3 px-4 py-1.5 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 text-[11px] font-sans text-zinc-400 shrink-0">
          <span className={`w-1.5 h-1.5 rounded-full ${loading ? 'bg-amber-400 animate-pulse' : error ? 'bg-red-400' : tailing ? 'bg-emerald-500 animate-pulse' : 'bg-emerald-500'}`} />
          {loading
            ? 'Loading…'
            : error
              ? 'Error'
