// app/notes/page.tsx - YOUR SCHEMA (userEmail)
import { db } from '@/lib/db';

export default async function NotesPage() {
  // ✅ Filter by current user email (server-side)
  const notes = await db.note.findMany({
    where: {
      userEmail: 'jonat@gmail.com', // Replace with session.user.email
    },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {notes.map((note) => (
          <div key={note.id} className="bg-gradient-to-br from-purple-50 to-indigo-50 p-6 rounded-2xl shadow-lg border border-purple-100 hover:shadow-xl transition-all group">
