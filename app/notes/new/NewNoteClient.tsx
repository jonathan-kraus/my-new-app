...
export default function NewNoteClient({ authorized, userId }: NewNoteClientProps) {
	const router = useRouter();
	const [content, setContent] = useState("");
	const [saving, setSaving] = useState(false);

	if (!authorized) {
		return <div className="p-6 text-red-400 text-lg">You must be signed in to create a note.</div>;
	}
