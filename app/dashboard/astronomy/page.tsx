...
export default async function DashboardAstronomyPage() {
	...
	if (!solar || !lunar) {
		return <div className="p-4 text-gray-500"> No astronomy data available yet. </div>;
	}

	const solarNoon = computeSolarNoon(solar!.sunrise.dateObj, solar!.sunset.dateObj);
	...
	return (
		<div className="p-6 space-y-10">
				...
				<div className="p-6 bg-white/5 rounded-xl border border-white/10 backdrop-blur">
					...
					<div className="space-y-2 text-sm">
						...
						<div className="flex justify-between">
							...
							<span>{formatInTimeZone(solarNoon, "America/New_York", "h:mm a")}</span>
						</div>
						<div className="flex justify-between">
							<span>Sunset</span>
			...
			{/* Full-width timeline */}
			<AstronomyTimeline
				sunrise={solar.sunrise?.timestamp ? parseISO(solar.sunrise.timestamp) : null}
				sunset={solar.sunset?.timestamp ? parseISO(solar.sunset.timestamp) : null}
				moonrise={lunar.moonrise?.timestamp ? parseISO(lunar.moonrise.timestamp) : null}
				moonset={lunar.moonset?.timestamp ? parseISO(lunar.moonset.timestamp) : null}
				sunriseBlueStart={
