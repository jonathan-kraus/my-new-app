 ...
 * @LastEditTime: 2026-03-20 13:55:01
 */
...
export async function logit(
	...
	const canonicalLine =
		normalizeLine(payload.line) ?? normalizeLine(meta.line) ?? normalizeLine(rawLine) ?? payload.line ?? meta.line ?? (rawLine && !isNaN(Number(rawLine)) ? Number(rawLine) : null);
	...
	const message = originalMessage ? `#${eventIndex} ${originalMessage}` : `#${eventIndex} JMSG`;

	// --- Canonical user/session extraction -----------------------------------
	const canonicalUserId = payload.userId ?? payload.session?.user?.id ?? meta.built?.userId ?? null;

	const canonicalSessionEmail =
		payload.sessionEmail ?? payload.session?.user?.email ?? meta.built?.sessionEmail ?? null;

	const canonicalSessionUser =
		payload.sessionUser ?? payload.session?.user?.name ?? meta.built?.sessionUser ?? null;
